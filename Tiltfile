# Tiltfile for GridForge local dev hot-reload
load("ext://restart_process", "docker_build_with_restart")

allow_k8s_contexts(["kind-gridforge", "minikube", "docker-desktop"])

local_resource("buf-generate", cmd="buf generate", deps=["proto/"])

services = ["api-gateway", "identity-service", "platform-bff"]
for s in services:
    docker_build("gridforge/" + s, "src/" + s, dockerfile="src/" + s + "/Dockerfile")
