...
spark = SparkSession.builder.appName("gridforge-segment").getOrCreate()
df = spark.read.format("iceberg").load("gridforge_mart.user_features")
...
preds.select("user_id","prediction").writeTo("gridforge_mart.user_segments").using("iceberg").createOrReplace()
